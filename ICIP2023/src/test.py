...
from .utils import get_order_typ_list, update_buffer, select_references
...
def load_models(config:DictConfig):
    ...
    for i in range(config.levels):
        im = elic.ELIC()
        checkpoint = torch.load(config.intra_models[i], map_location=lambda storage, loc: storage)
        im.load_state_dict(checkpoint['state_dict'])
        im = im.to(config.device).float()
        im.eval()
        intra_models.append(im)
        
    model = m.DeformB().to(config.device).float()
