...
class TestInfographic():
    ...
    def __init__(self, levels_intervals, folder_names):
        ...
        self.column_names = ["video", "level", "interval", "frame_num",
                             "frame_type", "psnr", "size", "pixels"]
        self.frame_df = pd.DataFrame(columns=self.column_names)
                
    def update(self, frame_type, frame_num, level, interval, video, psnr, size, pixels):
        data_point = {"video": video, "level": level, "interval": interval, 
                      "frame_type": frame_type, "frame_num": frame_num, "psnr": psnr, 
    ...
    def print_per_level_frame_num(self):
        grouped = self.frame_df.groupby(["level", "interval", "frame_num"])
        ...
        for (level_interval_frame_num, sums), (_, num_frames) in zip(self.totals_per_level_frame_num.iterrows(), 
                                                                       self.num_frames_per_level_frame_num.iteritems()):
            level, interval, frame_num = level_interval_frame_num
            avg_psnr = sums["psnr"]/num_frames
            avg_bpp = sums["size"]/sums["pixels"]
            logging.info("Frame Num: " + str(frame_num) + ", Level: " + str((level, interval)) + " PSNR: " + str(avg_psnr))
            logging.info("Frame Num: " + str(frame_num) + ", Level: " + str((level, interval)) + " bpp: " + str(avg_bpp))
