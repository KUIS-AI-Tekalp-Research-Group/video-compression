...
from utils import (MSE, PSNR, TestInfographic, UVGTestDataset,
                   compressai_image_compress, float_to_uint8, load_model)
...
parser.add_argument("--model_name", type=str, default="full_curve_final")                       # Model name
          
parser.add_argument("--test_path", type=str, default="/scratch/users/ecetin17/UVG/full_test/")  # Dataset paths
...
parser.add_argument("--b_pretrained", type=str, default="../BidirRef_continued.pth")            # Load model from this file
parser.add_argument("--i_pretrained", type=str, default="../mbt2018_gained_6level.pth")         # Load model from this file
...
""" qualities = [
    {-1: 1.0, 0: 1.0, 1: 0.66, 2: 0.33, 3: 0}, {-1: 1.33, 0: 1.33, 1: 1.0, 2: 0.66, 3: 0.33}, 
    {-1: 1.66, 0: 1.66, 1: 1.33, 2: 1.0, 3: 0.66}, {-1: 2.0, 0: 2.0, 1: 1.66, 2: 1.33, 3: 1.0}, 
    {-1: 2.33, 0: 2.33, 1: 2.0, 2: 1.66, 3: 1.33}, {-1: 2.66, 0: 2.66, 1: 2.33, 2: 2, 3: 1.66},
    {-1: 3.0, 0: 3.0, 1: 2.66, 2: 2.33, 3: 2.0}, {-1: 3.0, 0: 3.0, 1: 3.0, 2: 2.66, 3: 2.33}, 
    {-1: 3.0, 0: 3.0, 1: 3.0, 2: 3.0, 3: 2.66}, {-1: 3.0, 0: 3.0, 1: 3.0, 2: 3.0, 3: 3.0}
    ] """

qualities = [
    {-1: 0.0, 0: 1.0, 1: 0.66, 2: 0.33, 3: 0.0}, {-1: 0.33, 0: 1.33, 1: 1.0, 2: 0.66, 3: 0.33}, 
    {-1: 0.66, 0: 1.66, 1: 1.33, 2: 1.0, 3: 0.66}, {-1: 1.0, 0: 2.0, 1: 1.66, 2: 1.33, 3: 1.0}, 
    {-1: 1.33, 0: 2.33, 1: 2.0, 2: 1.66, 3: 1.33}, {-1: 1.66, 0: 2.66, 1: 2.33, 2: 2.0, 3: 1.66}, 
    {-1: 2.0, 0: 3.0, 1: 2.66, 2: 2.33, 3: 2.0}, {-1: 2.33, 0: 3.33, 1: 3.0, 2: 2.66, 3: 2.33}, 
    {-1: 2.66, 0: 3.66, 1: 3.33, 2: 3.0, 3: 2.66}, {-1: 3.0, 0: 4.0, 1: 3.66, 2: 3.33, 3: 3.0}, 
    {-1: 3.33, 0: 4.33, 1: 4.0, 2: 3.66, 3: 3.33}, {-1: 3.66, 0: 4.66, 1: 4.33, 2: 4.0, 3: 3.66}, 
    {-1: 4.0, 0: 5.0, 1: 4.66, 2: 4.33, 3: 4.0}, {-1: 4.33, 0: 5.0, 1: 5.0, 2: 4.66, 3: 4.33}, 
    {-1: 4.66, 0: 5.0, 1: 5.0, 2: 5.0, 3: 4.66}, {-1: 5.0, 0: 5.0, 1: 5.0, 2: 5.0, 3: 5.0} 
    ]


#args.levels_intervals = []
#for level in range(0, 3):
#    for interval in np.arange(1., 0., -0.25):
...
def mbt_image_compress(im_batch, model, n):
...
def test(b_model, i_model, device, args):
    ...
    with torch.no_grad():
        ...
        infographic = TestInfographic(6, folder_names)        
        ...
        for folder in folder_names:
            ...
            for idx, gop in enumerate(test_loader):
                ...
                for qual_dict in qualities:
                    level = qual_dict[-1]

                    i_quality = torch.tensor([qual_dict[-1]]).to(device)
                 
                    if idx == 0:
                        decoded[level] = {}
                        dec0, _, size0 = mbt_image_compress(gop[0], i_model, i_quality)
                        decoded[level][0] = dec0
                        ...
                        infographic.update("I", 0, level, folder, psnr0, size0.item(), 
                                           uint8_real0.shape[0] * uint8_real0.shape[1])
                    
                    if (idx + 1)%args.i_interval == 0:
                    
                        dec_last, _, size_last = mbt_image_compress(gop[-1], i_model, i_quality)
                        frame_type = "I"
                        frame_num = 0
                    else:
                        pass
                    
                    decoded[level][coding_order[1]] = dec_last
                    ...
                    infographic.update(frame_type, frame_num, level, folder, psnr_last, size_last.item(), 
                    ...
                    for order in coding_order_eff:
                        output = b_model(
                            ...
                            n=torch.tensor([qual_dict[hier_levels[order]]]).to(device),
                            train=False
                        )
                        ...
                        infographic.update("B", frame_num, level, folder, cur_psnr, 
...
def main(args):
    ...
    # Build I model
    i_model = gained_mbt2018(N=192, M=320, n=6).to(device).float()
    ...
    # Build B model
    b_model = BidirFlowRef(n=6, N=128).to(device).float()
