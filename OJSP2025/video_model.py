...
class DMC(CompressionModel):
    ...
    def forward_one_frame(self, x, dpb, q_index=None, fa_idx=0, video_name="video", frame_number=0):
        mv_y_q_enc, mv_y_q_dec, y_q_enc, y_q_dec = self.get_all_q(q_index)
        index = self.get_index_tensor(0, x.device)

        #est_mv = self.optic_flow(x, dpb["ref_frame"])
        est_mv, best_down_ratio = self.optimize_down_sampling_ratio(x, dpb, mv_y_q_enc, mv_y_q_dec, fa_idx, y_q_enc, y_q_dec, index, q_index)
        magnitude_scale = best_down_ratio
        mv_y = self.mv_encoder(est_mv/magnitude_scale, dpb["ref_mv_feature"], mv_y_q_enc)
        ...
        mv_hat, mv_feature = self.mv_decoder(mv_y_hat, mv_y_q_dec)
        mv_hat = mv_hat * magnitude_scale
        # Calculate motion encoding loss
        motion_encoding_loss = torch.mean((mv_hat - est_mv) ** 2).item()

        # Calculate average motion magnitude
        motion_magnitude = torch.sqrt(mv_hat[:, 0, :, :]**2 + mv_hat[:, 1, :, :]**2)
        avg_motion_magnitude = torch.mean(motion_magnitude).item()
        
        # Calculate prediction PSNR
        x_pred_decoder = self.warp(dpb["ref_frame"], mv_hat)
        psnr_prediction_decoder = PSNR(x, x_pred_decoder)
        x_pred_encoder = self.warp(dpb["ref_frame"], est_mv)
        psnr_prediction_encoder = PSNR(x, x_pred_encoder)
        # Determine grandparent directory name
        current_file_path = os.path.abspath(__file__)
        grandparent_dir = os.path.basename(os.path.dirname(os.path.dirname(current_file_path)))

        csv_filename = f"{grandparent_dir}_{video_name}.csv"
        ...
        final_psnr = PSNR(x, x_hat)
        with open(csv_filename, mode='a', newline='') as file:
            writer = csv.writer(file)
            if not file_exists:
                writer.writerow([
                    "Frame Number", "Motion Encoding Loss", "Avg Motion Magnitude", 
                    "Best Down Ratio", "PSNR Whole", "PSNR Prediction Decoder", "PSNR Prediction Encoder", "BPP Y", "BPP Z", 
                    "BPP MV Y", "BPP MV Z", "BPP Total"
                ])
            writer.writerow([frame_number, motion_encoding_loss, avg_motion_magnitude, best_down_ratio, final_psnr.item(), psnr_prediction_decoder.item(), psnr_prediction_encoder.item(), bpp_y.item(), bpp_z.item(), bpp_mv_y.item(), bpp_mv_z.item(), bpp.item()])
        ...
        return {"dpb": {
                    ...
                    "ref_y": y_hat,
                    "ref_mv_y": mv_y_hat,
                    "ref_down_ratio": best_down_ratio,
                },
                "bit": bit,
                }
    def optimize_down_sampling_ratio(self, x, dpb, mv_y_q_enc, mv_y_q_dec, fa_idx, y_q_enc, y_q_dec, index, q_index):
        downsampling_ratios = [1, 1.25, 1.5, 1.75, 2, 2.25, 2.5, 2.75, 3, 3.25, 3.5, 3.75, 4, 4.25, 4.5, 4.75, 5, 5.25, 5.5, 5.75, 6, 6.25, 6.5, 6.75, 7, 7.25, 7.5, 7.75, 8, 8.25, 8.5, 8.75]
        best_psnr = -float('inf')
        best_est_mv_down = None
        best_ratio = None
        for ratio in downsampling_ratios:
            # Perform downsampling
            # Downscale the images
            x_down = F.interpolate(x, scale_factor=1/ratio, mode="bilinear", antialias=True)
            ref_frame_down = F.interpolate(dpb["ref_frame"], scale_factor=1/ratio, mode="bilinear", antialias=True)
            
            # Pad the tensors to be multiples of p
            p = 8  # You can adjust this value as needed
            _, _, h, w = x_down.size()
            pad_bottom = (p - h % p) % p
            pad_right = (p - w % p) % p
            x_down_padded = F.pad(x_down, (0, pad_right, 0, pad_bottom))
            ref_frame_down_padded = F.pad(ref_frame_down, (0, pad_right, 0, pad_bottom))
            # Estimate motion vectors on downsampled frames
            est_mv_down_padded = self.optic_flow(x_down_padded, ref_frame_down_padded)
            # Remove padding from the motion field
            est_mv_down = est_mv_down_padded[:, :, :h, :w]
            est_mv_down = F.interpolate(est_mv_down, size=(x.shape[2], x.shape[3]), mode="bilinear", antialias=True) * ratio
            x_hat = self.warp(dpb["ref_frame"], est_mv_down)
        
            psnr = PSNR(x, x_hat)
            print(f"Downsampling ratio: {ratio}, PSNR: {psnr.item():.2f} dB")

            if ratio == dpb["ref_down_ratio"]:
                prev_ratio_psnr = psnr
                prev_ratio_mv = est_mv_down
            
            if psnr > best_psnr:
                best_psnr = psnr
                best_est_mv_down = est_mv_down
                best_ratio = ratio
        bias = 0.1
        if (best_psnr - prev_ratio_psnr) < bias:
            if dpb["ref_down_ratio"] != best_ratio:
                print("Bias applied")
                best_est_mv_down = prev_ratio_mv
                best_ratio = dpb["ref_down_ratio"]

        
        print(f"Selected downsampling ratio: {best_ratio}, Best PSNR: {best_psnr.item():.2f} dB")
        return best_est_mv_down, best_ratio
